...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        w_q = self.w_q(q).view(batch_size, len_q, n_head, d_k)
        w_k = self.w_k(k).view(batch_size, len_k, n_head, d_k)
        w_v = self.w_v(v).view(batch_size, len_v, n_head, d_v)

        q, k, v = w_q.transpose(1, 2), w_k.transpose(1, 2), w_v.transpose(1, 2)
